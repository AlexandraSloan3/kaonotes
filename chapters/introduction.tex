...
我在準備 KAN 研究時重新整理了這份線性代數筆記。本章沿用
Gilbert Strang 的 \emph{Introduction to Linear Algebra} 與 MIT 18.06
中的行、列兩種觀點，保留我自己的推導、計算，以及和後續程式實驗
直接相關的部分；內容不是教材摘錄。章名用繁體中文，節名用簡體中文，
正文以英文為主，讓這一章同時成為 KaoNotes 多語排版的可編譯範例。
...
\section[\zhHans 矩阵--向量乘积]{\zhHans 矩阵--向量乘法}
...
The same calculation can be expressed directly with PyTorch tensors:

\begin{lstlisting}[language=Python, caption={PyTorch tensor implementation}]
import torch
...
\section[\zhHans 可达性与可逆性]{\zhHans 可达性、线性无关与可逆性}
...
\section[\zhHans 与 KAN 研究的联系]{\zhHans 与 KAN 研究的联系}
